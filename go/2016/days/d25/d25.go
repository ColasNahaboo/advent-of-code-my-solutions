// Adventofcode 2016, d25, in go. https://adventofcode.com/2016/day/25
...
// Code mainly copied from day 23
...
var opnames = []string{"cpy", "inc", "dec", "jnz", "tgl", "out"}
...
func part1(lines []string) int {
	...
	valid := false
	init := 1
	lensignal := 1000			// overkill: in our input, 7 was sufficient
	for {
		VPf("  Testing for %d ticks with init=%d\n", lensignal, init)
		if valid = execEmit(code, [4]int{init,0,0,0}, lensignal); valid {
			break
		}
		init++
	}
...
// execEmit executes the code, but on each OUT instruction, check that we emit
// the proper signal compared to the last one: 0 then 1 or 1 then 0
// stop after siglen OUTs, we suppose if it was OK till now it will be OK forever

func execEmit(code []Instr, regs [4]int, siglen int) bool {
	codeSize := len(code)
	lineno := 0
	lastEmit := 1
	for lineno < codeSize {
		...
		case OUT:				// emit signal and check it alternates
			emit := evalValue(i.x, regs)
			if lastEmit * emit != 0 || lastEmit + emit != 1 {
				return false
			}
			lastEmit = emit
			siglen--
	...
	return false				// we do not loop? fail
}
