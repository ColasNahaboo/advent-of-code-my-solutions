...
func MakeEnumInt(from, n int) (enum []int) {
	for i := range n {
		enum = append(enum, from + i)
	}
	return
}	
...
// resize functions
// Reminder: truncating s to l is done by s = s[:l]

// Extends slice to a lengthl, returning a potentially new slice
// ex: slice = SliceExtend[int](slice, 234)
func SliceExtend[T any](s []T, l int) []T {
	if d := l - len(s); d > 0 {
		x := make([]T, d)
		s = append(s, x...)
	}
...
// Set an element at pos, eXtending it if needed
// ex: slice = SliceSetX[int](slice, pos, elt)
func SliceSetX[T any](s []T, p int, e T) []T {
	d := p - len(s)
	if d > 0 {					// append filler to desired length
		x := make([]T, d+1)
		s = append(s, x...)
	} else if d == 0 {			// optimisation if a simple append
		return append(s, e)
	}
	s[p] = e
	return s
...
// Get an element at pos, with success status
// ex: if elt, ok := SliceGetX[int[(slice, pos); ok { ...
func SliceGetX[T any](s []T, p int) (res T, status bool) {
	if p < len(s) {
		return s[p], true
	}
	return
...
// Get an element at pos, or provided default
// ex: SliceGetDef[int](slice, pos, -1)
func SliceGetDef[T any](s []T, p int, def T) T {
	if p < len(s) {
		return s[p]
	}
	return def
...
////// Convenience panic
func panicf(f string, v ...interface{}) bool {
	panic(fmt.Sprintf(f, v...))
	return true		   // this allows to call return panicf to satisfy compiler
}
