...
func BoundingBox(ps []Point) (x, y, w, h int) {
	...
	return x, y, xw - x + 1, yh - y + 1 // +1 as intervals are [...[
}
...
func MakeBoard[T comparable](w, h int) *Board[T] {
	var b Board[T]
	b.w = w
	b.h = h
	b.a = make([][]T, w)
...
// Set a valid point. To dynamically extend the board, see Append
...
func (b *Board[T]) Map(f func (b *Board[T], p Point) T) (bp *Board[T]) {
	bb := MakeBoard[T](b.w, b.h)
	for x := range b.w {
		for y := range b.h {
			bb.a[x][y] = f(b, Point{x, y})
		}
	}
	return bb
}

// apply f to all cells, and returns a new Board of the results, XY params
func (b *Board[T]) MapXY(f func (b *Board[T], x, y int) T) (bp *Board[T]) {
	bb := MakeBoard[T](b.w, b.h)
	for x := range b.w {
		for y := range b.h {
			bb.a[x][y] = f(b, x, y)
...
func (b *Board[T]) Apply (f func (b *Board[T], p Point)) {
	for x := range b.w {
		for y := range b.h {
			f(b, Point{x, y})
		}
...
// apply f to all cells, that can modify them in place if needed. XY params.
func (b *Board[T]) ApplyXY (f func (b *Board[T], x, y int)) {
	for x := range b.w {
		for y := range b.h {
			f(b, x, y)
		}
...
// apply f to all cells, in natural row-major order (incr x then y)
func (b *Board[T]) Apply2D (f func (b *Board[T], p Point)) {
	for y := range b.h {
		for x := range b.w {
			f(b, Point{x, y})
		}
...
// apply f to all cells, in natural row-major order (incr x then y), XY params.
func (b *Board[T]) Apply2DXY (f func (b *Board[T], x, y int)) {
	for y := range b.h {
		for x := range b.w {
			f(b, x, y)
		}
...
// fast Copy of another board into the existing board

func (b *Board[T])Copy(src *Board[T]) {
	if src.w != b.w || src.h != b.h {
		panic(fmt.Sprintf("Cannot copy a %dx%d board into a %dx%d one", src.w, src.h, b.w, b.h))
	}
    for x := range src.w {
		copy(b.a[x], src.a[x])
    }
...
func (b *Board[T])Equal(src *Board[T]) bool {
	if src.w != b.w || src.h != b.h {
		return false
	}
    for x := range b.w {
		for y := range b.h {
			if b.a[x][y] != src.a[x][y] {
				return false
			}
		}
    }
...
// resize the board: Extend or Shrink to go to the exact w and h
func (b *Board[T]) Resize(w, h int) {
	switch {
	case w == b.w && h == b.h: return
	case w >= b.w && h >= b.h: b.Extend(w, h)
	case w <= b.w && h <= b.h: b.Shrink(w, h)
	case w < b.w:
		b.Shrink(w, b.h)
		b.Extend(w, h)
	default:
...
// extend if w or h bigger, do nothing otherwise
func (b *Board[T]) Extend(w, h int) {
	if h > b.h {				// expand all columns
		for x := 0; x < b.w; x++ {
			b.a[x] = append(b.a[x], make([]T, h - b.h)...)
		}
		b.h = h
	}
	if w > b.w {				// expand the number of columns
		for _ = range w - b.w {
			b.a = append(b.a, make([]T, b.h))
		}
		b.w = w
	}
}

// shrink if w or h is smaller, do nothing otherwise
func (b *Board[T]) Shrink(w, h int) {
	if h < b.h {				// truncate all columns
		for x := 0; x < b.w; x++ {
			b.a[x] = b.a[x][:h]
		}
		b.h = h
	}
	if w < b.w {				// remove columns
		b.a = b.a[:w]
		b.w = w
	}
}

// Set p to v, extending the board if needed
func (b *Board[T]) Append(p Point, v T) {
	b.Extend(p.x+1, p.y+1)
	b.a[p.x][p.y] = v
}
...
func (b *Board[T]) SetRow(y int, row []T) {
	for x := range b.a {
		b.a[x][y] = row[x]
	}
}
...
// get or set col x
func (b *Board[T]) GetCol(x int) []T {
	return b.a[x]
}
func (b *Board[T]) SetCol(x int, col []T) {
	b.a[x] = col
}

// insert & delete rows & columns

func (b *Board[T]) InsertRowsBefore(y, n int) {
	colsegment := make([]T, n)
	for x := range b.a {
		b.a[x] = append(b.a[x], colsegment...)
		copy(b.a[x][y+n:], b.a[x][y:b.h])
		copy(b.a[x][y:], colsegment)
	}
	b.h += n
}

func (b *Board[T]) InsertColsBefore(x, n int) {
	xcols := make([][]T, n)
	b.a = append(b.a, xcols...)	// make room
	for i := range (b.w - x) {
		b.a[b.w - 1 + n - i] = b.a[b.w - 1 - i]
	}
	for i := range n {
		b.a[x - 1 + n - i] = make([]T, b.h)
	}
	b.w += n
}

func (b *Board[T]) DeleteRowsAt(y, n int) {
	for x := range b.a {
		b.a[x] = slices.Delete(b.a[x], y, y+n)
	}
	b.h -= n
}

func (b *Board[T]) DeleteColsAt(x, n int) {
	b.a = slices.Delete(b.a, x, x+n)
	b.w -= n
}
...
func BFSInit(b *Board[bool]) (bfs *Board[int]) {
	bfs = MakeBoard[int](b.w, b.h)
	bfsSeed := bfs.ClearInitValue(-1) //  fill with -1 as default
	bfs.Clear(bfsSeed)
	return
}
...
func ParseCellChars(x, y int, r rune) string { // chars
	return string(r)
}

var boardStart, boardEnd Point	// globals set by chars "S" and "E" in bool map
func ParseCellBoolSE(x, y int, r rune) bool { // ".#SE"
		...
		default: panic(fmt.Sprintf("Bad cell glyph: %v", r))
		}
		return false
}
...
// Apply printcell that must return a string to all cells in "2D' order (rows)

func (b *Board[T]) VPcells(title string, printcell func(c T) string) {
	if !verbose {
		return
	}
...
// Simpler version, just Printing the cells

func (b *Board[T]) VP(title string) {
	if !verbose {
		return
	}
	fmt.Printf("Board %dx%d %s\n", b.w, b.h, title)
