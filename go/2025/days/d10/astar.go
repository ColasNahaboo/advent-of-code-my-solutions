...
//////////// The Types you must provide:
// Graph is a pointer to any type holding the context you need for the Calllbacks
// Node is a Comparable type that identifies a node, its ID

// then a Path is a sequence of nodes in a graph.
type Path[Node any] []Node

//////////// The path-finding functions:
// AStarFindPath, AStarFindPathLess, AStarFindPathSteps
...
func AStarFindPath[Graph any, Node comparable](g Graph, start, dest Node,
	cf ConnectedFunc[Graph, Node], df, hf CostFunc[Graph, Node],
	ne EndFunc[Graph, Node]) Path[Node] {
...
// same, but only consider paths less than maxcost to optimize search

func AStarFindPathLess[Graph any, Node comparable](g Graph, start, dest Node,
	cf ConnectedFunc[Graph, Node], df, hf CostFunc[Graph, Node],
	ne EndFunc[Graph, Node], maxcost float64) Path[Node] {
	closed := make(map[Node]bool)
...
// same, but only consider paths with less than maxlen steps to optimize search
// (start & end included)

func AStarFindPathSteps[Graph any, Node comparable](g Graph, start, dest Node,
	cf ConnectedFunc[Graph, Node], df, hf CostFunc[Graph, Node],
	ne EndFunc[Graph, Node], maxlen int) Path[Node] {
	closed := make(map[Node]bool)
...
//////////// User-usable Convenience functions

// A CostFunc to use if your connected nodes at are the same distance of 1

func AdjacentDist[Graph any, Node comparable](g Graph, n1, n2 Node) float64 {
	return 1.0
}

//////////// Internals, feel free to use but may (improbably) changes
