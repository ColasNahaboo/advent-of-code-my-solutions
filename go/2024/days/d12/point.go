...
func (p Point) MoveOrtho(dir int) Point {
	return Point{p.x + DirsOrtho[dir].x, p.y + DirsOrtho[dir].y}
}

func (p Point) MoveDiag(dir int) Point {
	return Point{p.x + DirsDiag[dir].x, p.y + DirsDiag[dir].y}
}

func (p Point) MoveAll(dir int) Point {
	return Point{p.x + DirsAll[dir].x, p.y + DirsAll[dir].y}
}
...
// Coords inside Board
func (b *Board[T]) InsideXY(x, y int) bool {
	return x >= 0 && x < b.w && y >= 0 && y < b.h
}

// apply f to all cells, and returns ("maps") a new Board of the results
func (b *Board[T])Map (f func (b *Board[T], x, y int, v T) T) (bp *Board[T]) {
...
func (b *Board[T])Apply (f func (b *Board[T], x, y int, v T)) {
	for x, col := range b.a {
		for y, v := range col {
			f(b, x, y, v)
		}
