...
//////////// Crawlers to adjacent cells

// free must tell is a point is free (valid to crawl into) on the board

func (b *Board[T]) AdjsOrtho(p Point,  free func(b *Board[T], p Point) bool) []Point {
	ao := make([]Point, 0, 4)
	for d := range 4 {
		if q := p.StepOrtho(d); free(b, q) {
			ao = append(ao, q)
		}
	}
	return ao
...
func (b *Board[T]) AdjsAll(p Point,  free func(b *Board[T], p Point) bool) []Point {
	ao := make([]Point, 0, 8)
	for d := range 8 {
		if q := p.StepAll(d); free(b, q) {
			ao = append(ao, q)
		}
	}
	return ao
...
// functions, as we cannot specialise on sub-types of T with methods

// on a bool board where true => wall, free adjs cells in the 4 ortho dirs
func AdjsOrtho(b *Board[bool], p Point) []Point {
	ao := make([]Point, 0, 4)
	for d := range 4 {
		if q := p.StepOrtho(d); ! b.GetOr(q, true) {
			ao = append(ao, q)
		}
	}
	return ao
...
// on a bool board where true => wall, free adj cells in the 8 ortho+diag dirs
func AdjsAll(b *Board[bool], p Point) []Point {
	ao := make([]Point, 0, 8)
	for d := range 8 {
		if q := p.StepOrtho(d); ! b.GetOr(q, true) {
			ao = append(ao, q)
...
//// BFS functions not needing a pre-exploration via BFSfill

// returns list of shortest paths from p to end on a bool Board with true=wall
// and their length
// Caveat: paths include both p and end
// Note: we reverse-chain so that the linked list of a path is in order

func BFSMinPathsBool(b *Board[bool], end, p Point) (minpaths []*PointChain, minpathlen int) {
	minpathlen = -1
	startchain := PointChain{p: p}
	// keep track of the current path so we do not loop
	visits := MakeBoard[bool](b.w, b.h)
	visits.Set(p, true)
	path := &startchain
	// for recursive lambda, we must first declare a variable explicitely
	var explore func(*Board[bool], *Board[bool], *PointChain, Point, Point, int)
	explore = func(b *Board[bool], visited *Board[bool], path *PointChain, p, end Point, pathlen int) {
		if p == end {
			if pathlen < minpathlen || minpathlen < 0 { // restarts collect of minpaths
				minpathlen = pathlen
				minpaths = []*PointChain{path}
			} else if pathlen == minpathlen {
				minpaths = append(minpaths, path)
			}
			return
		}
		for _, q := range AdjsOrtho(b, p) {
			if visits.Get(q) {
				continue
			}
			visits.Set(q, true)
			explore(b, &visits, &PointChain{q, path}, q, end, pathlen+1)
			visits.Set(q, false)
		}
	}
	explore(b, &visits, path, p, end, 0)
	return
}

// same, but works on any Board via a lambda to test if a point is free 

func (b *Board[T]) BFSMinPaths(end, p Point, free func(b *Board[T], p Point) bool) (minpaths []*PointChain, minpathlen int) {
	minpathlen = -1
	startchain := PointChain{p: p}
	// keep track of the current path so we do not loop
	visits := MakeBoard[bool](b.w, b.h)
	...
	path := &startchain
	// for recursive lambda, we must first declare a variable explicitely
	var explore func(*Board[T], *Board[bool], *PointChain, Point, Point, int)
	explore = func(b *Board[T], visited *Board[bool], path *PointChain, p, end Point, pathlen int) {
		if p == end {
			if pathlen < minpathlen || minpathlen < 0 { // restarts collect of minpaths
				minpathlen = pathlen
				minpaths = []*PointChain{path}
		...
		for _, q := range b.AdjsOrtho(p, free) {
			if visits.Get(q) {
				continue
			}
			visits.Set(q, true)
...
func (pc *PointChain) VP(title string) {
	if !verbose {
		return
	}
	fmt.Printf("%s:", title)
